...
function Body({ body = [] }) {
  return body.map((n, i) => {
    if (n._type === "richImage") {
      return <div key={i} style={{ margin: "0 0 32px" }}><Media value={n} ratio="16/9" w={1400} h={790} /></div>;
    }
    if (n._type === "pullQuote") {
      return (
        <div className="v2-quote-block" key={i} style={{ margin: "0 0 32px" }}>
  ...
  return (
    <>
      ...
      <div className="wrap" style={{ maxWidth: 900, paddingTop: 24 }}>
        <div className="v2-tag" style={{ marginBottom: 18 }}>{S(post.tag).text}</div>
