...
export async function getCategories() {
  return (await q(
    `*[_type=="productCategory" && visible != false && defined(slug.current)]|order(orderRank asc){
      _id, _type, name, "slug": slug.current, blurb, description, "coverUrl": coverImage.asset->url,
      "products": *[_type=="productItem" && references(^._id) && visible != false && defined(slug.current)]|order(orderRank asc){
        _id, _type, name, "slug": slug.current, material, "thumbUrl": photos[0].asset->url
      }
    }`
...
export async function getCategory(slug) {
  return q(
    `*[_type=="productCategory" && slug.current==$slug][0]{
      _id, _type, name, "slug": slug.current, blurb, description, "coverUrl": coverImage.asset->url,
      "products": *[_type=="productItem" && references(^._id) && visible != false && defined(slug.current)]|order(orderRank asc){
        _id, _type, name, "slug": slug.current, material, moq, "thumbUrl": photos[0].asset->url
      }
    }`,
    { slug }
  );
...
export async function getProduct(slug) {
  return q(
    `*[_type=="productItem" && slug.current==$slug][0]{
      _id, _type, name, "slug": slug.current, description, material, moq,
      "category": category->{ name, "slug": slug.current },
      "photos": photos[]{ caption, "url": asset->url },
      specs[]{ label, value },
      variants[]{ _key, material, technique, moq, designer,
...
export async function getRole(slug) {
  return q(
    `*[_type=="jobRole" && slug.current==$slug][0]{
      _id, _type, role, "slug": slug.current, dept, location, type, summary,
...
export async function getPosts() {
  return (await q(
    `*[_type=="blogPost" && visible != false && defined(slug.current)]|order(orderRank asc){
      _id, _type, title, "slug": slug.current, tag, date, readTime, excerpt,
...
export async function getPost(slug) {
  return q(
    `*[_type=="blogPost" && slug.current==$slug][0]{
      _id, _type, title, "slug": slug.current, tag, date, readTime, excerpt,
      cover{ "url": image.asset->url, alt, caption },
      body[]{ _key, ..., _type == "richImage" => { "url": image.asset->url, alt, caption } },
...
export async function getV2Site() {
  const [site, audiences, categories, nav] = await Promise.all([
    ...
    q(`*[_id=="navigation"][0]{
      _id, _type,
      mainNav[]{ label, href, autoMenu, children[]{ _key, label, href } },
