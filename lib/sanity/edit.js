...
/**
 * Strips stega metadata from a value.
 *
 * In draft mode every string from Sanity carries invisible characters so the
 * Studio can trace it back to its field. That is correct for text a human
 * reads, but corrupts any string used as a *machine* value — a form field's
 * name attribute, an element id, a CSS keyword. A polluted field name silently
 * breaks form submission and validation inside Presentation.
 *
 * Use this wherever a Sanity string becomes an attribute, key or comparison
 * rather than visible copy.
 */
export { stegaClean as clean } from "next-sanity";
