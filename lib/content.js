...
// The v2 dataset has a public ACL, so reads work without a token. A token is
// only needed for private datasets; without one we still try the fetch and let
// the catch below fall back to JSON if the dataset turns out to be unreadable.
