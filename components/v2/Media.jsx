...
export default function Media({
  value, ratio = "4/3", className = "", w = 1200, h = 900,
  contain = false, showCaption = true,
  // Spread from editAttr() so clicking the image in Presentation opens the
  // field that owns it. An image URL cannot carry stega metadata the way text
  // does, so the mapping has to be declared explicitly.
  edit,
}) {
  const url = value?.url;
  ...
  return (
    <div className={`v2-media ${contain ? "contain" : ""} ${className}`} style={{ aspectRatio: ratio }} {...(edit || {})}>
      {url ? (
